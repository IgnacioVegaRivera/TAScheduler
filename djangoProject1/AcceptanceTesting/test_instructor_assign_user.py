...
class TestInstructorAssignUserAcceptance(TestCase):
    def setUp(self):
        ...
        self.admin = User.objects.create(first_name='Ad', last_name='Min', username='admin', password='password',
                                         email='admin@uwm.edu', phone_number="1234567890", address='313 W Street',
                                         skills='Admin skills', role='Admin')
        self.instructor = User.objects.create(first_name='Inst', last_name='Ructor', username='instructor',
                                              password='password', email='instructor@uwm.edu',
                                              phone_number="0123456789", address='404 Your Moms House',
                                              skills='Good at teaching', role='Instructor')
        self.ta = User.objects.create(first_name='T', last_name='A', username='ta', password='password',
                                      email='ta@uwm.edu',
                                      phone_number='6061912020', address='909 Next Door', skills='Proficient in java',
                                      role='TA')
    ...
    def test_assign_user_success(self):
        ...
        response = self.donkey.post('/edit_section_assignment.html',
                                    {"form_name": "edit_section_assignment", "section_id": self.labOne.id,
                                     "section_user": self.ta.id}, follow=True)
        self.labOne.refresh_from_db()
        self.assertEqual(response.status_code, 200)
        self.assertEqual(self.labOne.user, self.ta)
        self.assertIn('message', response.context)
        self.assertEqual(response.context['message'], "Section assignment was successfully updated")

    def test_assign_user_fail(self):
        self.login_as("instructor")
        ...
        self.assertIn('message', response.context)
        self.assertEqual(response.context['message'], "Failed to update the section assignment")
